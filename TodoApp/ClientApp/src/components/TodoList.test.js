...
const mockState = {
  todos: [
    { task: 'Test task 1', completed: false, deadline: null },
    { task: 'Test task 2', completed: true, deadline: '2024-02-20T10:00:00' },
  ],
};
...
function renderTodoList() {
    return render(
        <TodoContext.Provider value={{ state: mockState, dispatch: mockDispatch }}>
            <TodoList />
...
describe('TodoList component', () => {
  ...
  test('toggles completion status when the "mark as completed" button is clicked', () => {
    ...
    expect(mockDispatch).toHaveBeenCalledWith({ type: 'TOGGLE_COMPLETE', payload: mockState.todos[0] });
  });
  ...
  test('deletes a todo item when the delete button is clicked', () => {
    ...
    expect(mockDispatch).toHaveBeenCalledWith({ type: 'DELETE_TODO', payload: mockState.todos[0] });
  });
});
